...
public:
  ...
  static Addr symbol(const char *label);
  ...
  static void step();
  static void step_wdc();
  static void step_rockwell();
  static void step_l28();
...
private:
  ...
  // Direct Page Indirect Indexed - (d),X
  INLINE static Addr am_diix()
  {
    Byte disp = getByte(join(pbr, pc));

    BYTES(1);
    cycles += 3;
    return (bank(dbr) | (getWord(bank(0) | (dp.w + disp)) + x.w));
  }
  ...
  INLINE static void op_pea(Addr ea)
  {
	...
	// std::cout << std::hex << ea << "->" << getWord(ea) << std::endl;
    pushWord(getWord(ea));
    cycles += 5;
  }
  ...
  INLINE static void op_bbs(int bit, Addr n, Addr ea)
  {
    TRACE("BBS");
  ...
  INLINE static void op_bbr(int bit, Addr n, Addr ea)
  {
    TRACE("BBR");
