...
elf_context_t *elf_create(const char *filename, ELF_Half e_machine)
{
  ...
  context = new elf_context_t;

  context->filename = as_strdup(filename);
  context->ehdr = new ELF_Ehdr;
  ...
  elf_section_t *undef = elf_section_create(context, 0);
  undef->shdr->sh_type = SHT_NULL;
  undef->shdr->sh_flags = 0;
  ...
  elf_section_t *shstrtab = elf_section_create(context, 0);
  ...
  shdr->sh_entsize = 0;
  shdr->sh_size = 1;
  shstrtab->data = (unsigned char *)as_mallocz(1); // First 0 byte
  context->shstrtab = shstrtab;
  shdr->sh_name = elf_string_create(shstrtab, ".shstrtab");
  
  // Create the symbol table
  elf_section_t *symtab = elf_section_create(context, 0);
  shdr = symtab->shdr;
  ...
  symtab->data = (unsigned char *)new ELF_Sym; // First 0 entry
...
elf_context_t *elf_read(const char *filename) 
{
  ...
  if (fd != 0) {
    ...
    if ((read(fd, context->ehdr, sizeof(ELF_Ehdr)) != sizeof(ELF_Ehdr)) ||
      ...
      // Load the segments
      lseek(fd, context->ehdr->e_phoff, SEEK_SET);
      for (int i = 0; i < context->ehdr->e_phnum; i++) {
        elf_segment_t *segment = new elf_segment_t;
		segment->phdr = new ELF_Phdr;
        r = read(fd, segment->phdr, sizeof(ELF_Phdr));
	if (r != sizeof(ELF_Phdr)) as_error("Malformed ELF file %d", r);
	int num = i;
	int num_alloc = 1;
	...
	if ((num & (num - 1)) == 0) {
	  ...
	  context->segments = (elf_segment_t**)as_realloc(context->segments, num_alloc * sizeof(void*));
	}
	context->segments[num++] = segment;
      }

      // Now load the data
      for (int i = 0; i < context->ehdr->e_phnum; i++) {
	elf_segment_t *segment = context->segments[i];
	ELF_Word len = segment->phdr->p_filesz;
	segment->data = (unsigned char *)as_malloc(len);
	lseek(fd, segment->phdr->p_offset, SEEK_SET);
	// as_error("Seek to %0x", segment->phdr->p_offset);
	r = read(fd, segment->data, len);
	if (r != len) as_error("Malformed ELF segment");
	// printf("Read segment %d bytes.\n", len);
      }
    }
  }
  close(fd);
...
void elf_write_executable2(elf_context_t *context)
{
  ssize_t r;
  ...
  if (fd != 0) {
    ...
    */
    ...
    ELF_Phdr* segment_buf = new ELF_Phdr;
...
void elf_release(elf_context_t *context)
{
  if( context ) {
    ...
    for (int i = 0; i < context->ehdr->e_shnum; i++) {
      section = *sections;
      while (section->scope != 0) scope_pop(); // Frees all values
...
{
  elf_section_t *section = new elf_section_t;
  section->shdr = new ELF_Shdr;
  section->shdr->sh_name = name;
  
  section->instrs = 0;
  section->num_instrs = 0;
  section->data = 0;
  section->scope = new Scope;
  ...
  if ((num & (num - 1)) == 0) {
    if (num) num_alloc = num * 2;
    context->sections = (elf_section_t**)as_realloc(context->sections, num_alloc * sizeof(void*));
  }
...
ELF_Word elf_string_create(elf_section_t *section, const char *str) 
{
  ...
  if (!offset) {
    ...
    section->data = (unsigned char *)as_realloc(section->data, offset + len);
...
ELF_Word elf_symbol_locate(elf_section_t *section, ELF_Word st_name)
{
  ...
  do {
    if( p->st_name == st_name) return ((ELF_Word)((long)p - (long)section->data));
...
ELF_Word elf_relocation_create(elf_section_t *section, ELF_Addr offset, ELF_Word info)
{
  ...
  // We realloc every time, this could definitely be better
  section->data = (unsigned char *)as_realloc(section->data, off + len);
